...
// 历史记录查询
export default {
  namespace: 'v2_historyDataDetailed',
  state: {
    activeMarkInfo: {},
    teacherName: '',
    className: '',
  ...
  reducers: {
    ...
    queryBoxDetailSuccess(state, action) {
      ...
      if (!active && !isEmpty(data.lookUpMark)) {
        active = {
          activeImgUrl: data.lookUpMark[0].url,
          rate: data.lookUpMark[0].attendanceCount,
          date: data.lookUpMark[0].time,
    ...
    changeActiveTime(state, action) {
      ...
      if (action.isTag && action.isTag === 'attendanceTag') {
        ...
        let bbox = state.bboxs && state.bboxs[dateIndex]
